            ...
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-2xl border border-gray-100 p-8">
                ...
                <formac action= "{{ route ('articles.store') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-6">
                            ...
                            <input type="text" name="internal_code" id="internal_code" value="{{ old('internal_code') }}" maxlength="50" class="w-full rounded-xl border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 focus:ring focus:ring-indigo-500 focus:ring-opacity-20 shadow-sm transition-colors"placeholder="Ej. Desarrollo web">
                            @error('internal_code')
                                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>

                            <label for="detailed_description" class="block text-sm font-medium text-gray-700">Descripción Detallada</label>
                            <input type="text" name="detailed_description" id="detailed_description" value="{{ old('detailed_description') }}" maxlength="200" class="w-full rounded-xl border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 focus:ring focus:ring-indigo-500 focus:ring-opacity-20 shadow-sm transition-colors"placeholder="Ej. Desarrollo web">
                            @error('detailed_description')
                                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                            <label for="price" class="block text-sm font-medium text-gray-700">Precio</label>
                            <input type="number" name="price" id="price" value="{{ old('price') }}" maxlength="10" class="w-full rounded-xl border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 focus:ring focus:ring-indigo-500 focus:ring-opacity-20 shadow-sm transition-colors"placeholder="Ej. Desarrollo web">
                            @error('price')
                                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                         ...
                         <label for="available_status" class="block text-sm font-medium text-gray-700">Estado Disponible</label>
                            ...
                            <input type="text" name="entry_date" id="entry_date" value="{{ old('entry_date') }}" maxlength="10" class="w-full rounded-xl border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 focus:ring focus:ring-indigo-500 focus:ring-opacity-20 shadow-sm transition-colors"placeholder="Ej. Desarrollo web">
                            @error('entry_date')
                                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>

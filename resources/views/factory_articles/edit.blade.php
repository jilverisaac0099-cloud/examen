        ...
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-2xl border border-gray-100 p-8">
                ...
                <form action="{{ route('factory_articles.update', $factory_article) }}" method="POST" onsubmit="confirmarActualizacion(event)" novalidate>
                    ...
                    <div class="mb-6">
                        <label for="current_stock" class="block text-sm font-medium text-gray-700 mb-2">existencias actuales</label>
                        <input type="number" id="current_stock" name="current_stock" value="{{ old('current_stock', $factory_article->current_stock) }}" class="w-full rounded-xl border-gray-300 focus:border-indigo-500 focus:ring focus:ring-indigo-500 focus:ring-opacity-20 shadow-sm transition-colors">
                        @error('current_stock')
                    ...
                    <div class="mb-6">
                        ...
                        <select id="factory_id" name="factory_id" class="w-full rounded-xl border-gray-300 focus:border-indigo-500 focus:ring focus:ring-indigo-500 focus:ring-opacity-20 shadow-sm transition-colors">
                            ...
                            @foreach($factories as $factory)
                                <option value="{{ $factory->id }}" {{ old('factory_id', $factory_article->factory_id) == $factory->id ? 'selected' : '' }}>
                                    {{ $factory->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-6">
                        <label for="delivery_time" class="block text-sm font-medium text-gray-700 mb-2">existencias actuales</label>
                        <input type="text" id="delivery_time" name="delivery_time" value="{{ old('delivery_time', $factory_article->delivery_time) }}" class="w-full rounded-xl border-gray-300 focus:border-indigo-500 focus:ring focus:ring-indigo-500 focus:ring-opacity-20 shadow-sm transition-colors">
                        @error('delivery_time)
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>

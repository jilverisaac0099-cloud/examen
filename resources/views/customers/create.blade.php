            ...
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-2xl border border-gray-100 p-8">
                ...
                <form action="{{ route('customers.store') }}" method="POST" novalidate>
                    ...
                    <div class="mb-6">
                        ...
                        <input type="texto" id="credit_limit" name="credit_limit" value="{{ old('credit_limit') }}" step="0.01" class="w-full rounded-xl border-gray-300 focus:border-indigo-500 focus:ring focus:ring-indigo-500 focus:ring-opacity-20 shadow-sm transition-colors" placeholder="Ej. 100.00">
                        @error('credit_limit')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>

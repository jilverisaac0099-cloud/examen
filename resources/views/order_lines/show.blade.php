    ...
    <div class="py-12">
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-xl sm:rounded-2xl border border-gray-100 dark:border-gray-700 p-8">
                ...
                <div class="mb-6">
                    <h3 class="text-sm font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-2">Nombre del pedido</h3>
                    <p class="text-lg font-semibold text-gray-900 dark:text-white">{{ $order->d }}</p>
                </div>
                <div class="mb-6">
                    <h3 class="text-sm font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-2">Estado del pedido</h3>
                    <p class="text-lg font-semibold text-gray-900 dark:text-white">{{ $order->order_status }}</p>
                </div>
                <div class="mb-6">
                    <h3 class="text-sm font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-2">Cliente</h3>
                    <p class="text-lg font-semibold text-gray-900 dark:text-white">{{ $order->customer }}</p>
                </div>
                <div class="mb-6">
                    <h3 class="text-sm font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-2">Dirección de envío</h3>
                    <p class ="text-lg font-semibold text-gray-900 dark:text-white">{{ $order->shipping_address }}</p>
                </div>
                <div class="mb-6">
                    <h3 class="text-sm font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-2">subTotal</h3>
                    <p class="text-lg font-semibold text-gray-900 dark:text-white">{{ $order->subtotal }}</p>
                </div>
                <div class="mb-6">
                    <h3 class="text-sm font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-2">Total</h3>
                    <p class="text-lg font-semibold text-gray-900 dark:text-white">{{ $order->grand_total }}</p>
                </div>
                <div class="mb-6">
                    <h3 class="text-sm font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-2">Descuento</h3>
                    <p class="text-lg font-semibold text-gray-900 dark:text-white">{{ $order->additional_note }}</p>
                </div>
                <div class="mb-6">
                    <h3 class="text-sm font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-2">iba</h3>
                    <p class="text  -lg font-semibold text-gray-900 dark:text-white">{{ $order->iba }}</p>
                </div>
                ...
                <div class="flex items-center justify-end gap-4 mt-8 pt-6 border-t border-gray-100 dark:border-gray-700">
                    <form action="{{ route('orders.destroy', $order) }}" method="POST" class="inline" id="form-delete-{{ $order->id }}">
                        @csrf
                        @method('DELETE')
                        <button type="button" onclick="confirmarEliminacion({{ $order->id }})" class="inline-flex items-center px-4 py-2 bg-white dark:bg-gray-800 border border-red-200 dark:border-red-900/50 rounded-xl font-semibold text-xs text-red-600 dark:text-red-400 uppercase tracking-widest shadow-sm hover:bg-red-50 dark:hover:bg-red-900/20 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 transition ease-in-out duration-150">
                            Eliminar
                        </button>
                    </form>

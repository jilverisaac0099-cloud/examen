    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-2xl border border-gray-100">
                                ...
                                <tr class="hover:bg-gray-50 transition-colors group">
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="text-sm font-semibold text-gray-900">
                                            {{ $order_line->request_quantity ?? 'Sin stock actual' }}
                                        </div>
                                    </td>

                                    <td class="px-6 py-4">
                                        <div class="text-sm font-semibold text-gray-900">
                                            {{ $order_line->unite_price ?? 'Sin costo de negociación' }}
                                        </div>
                                    </td>
                                    ...
                                    <td class="px-6 py-4">
                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                                            {{ $factory_article->article->name ?? 'Sin categoría' }}
                                        </span>
                                    </td>
                                    <td class="px-6 py-4 text-right">
                                        <div class="flex justify-end items-center space-x-3 opacity-0 group-hover:opacity-100 transition-opacity">
                                            <a href="{{ route('order_lines.show', $order_line) }}" class="text-gray-400 hover:text-indigo-600 transition-colors" title="Ver">
                                            ...
                                            <form action="{{ route('order_Lines.destroy', $order_line) }}" method="POST" class="inline" id="form-delete-{{ $factory_article->id }}">
                                                @csrf
                                                @method('DELETE')
                                ...
                                <tr>
                                    <td colspan="4" class="px-6 py-12 text-center">
                                        <div class="flex flex-col items-center">
                                            ...
                                            <p class="text-gray-500 text-lg font-medium">No hay artículos de fábrica registrados</p>
                                        </div>
                                    </td>
                                </tr>
                            @endforelse
                ...
                @if(isset($order_line) && method_exists($order_lines, 'hasPages') && $order_lines->hasPages())
